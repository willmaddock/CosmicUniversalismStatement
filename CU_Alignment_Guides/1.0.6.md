...
# ===== Constants =====
BASE_CU = 3_079_900_000_000  # 4 BCE anchor (January 1, 4 BCE)
BASE_DATE = datetime(-4, 1, 1)  # Proleptic Gregorian calendar
SCALE_RATIO = Fraction(13_900_000_000, 2029)  # CU-years per Gregorian year
DAYS_PER_YEAR = Fraction(3652425, 10000)  # Mean tropical year (365.2425 days)
TOTAL_BREATH = 3_108_000_000_000  # Full cosmic breath duration

# ===== Core Converter =====
def cu_to_gregorian(
    cu_time: Union[float, int], 
    timezone: str = "UTC",
    verbose: bool = False
) -> str:
    """
    Convert CU-time to Gregorian date with exact arithmetic and cosmic context.
    
    Args:
        cu_time: CU-years since sub-ZTOM (must be ≥ BASE_CU)
        timezone: IANA timezone (e.g., "America/Denver" for MDT)
        verbose: If True, returns cosmic context with the date
        
    Returns:
        Formatted date string with timezone, or error message
        Includes cosmic context if verbose=True
    """
    try:
        # === Input Validation ===
        if cu_time < BASE_CU:
            return "Error: Pre-BTOM temporality (input < 3,079,900,000,000 CU-years)"
        if cu_time >= TOTAL_BREATH:
            return "ZTOM proximity alert (input ≥ 3.108 trillion CU-years)"
            
        # === Exact Conversion ===
        # 1. Calculate exact time difference using fractions
        cu_diff = Fraction(int(cu_time * 100), 100) - Fraction(BASE_CU, 1)
        
        # 2. Convert CU-years to Gregorian years
        gregorian_years = float(cu_diff / SCALE_RATIO)  # Final float conversion
        
        # 3. Convert years to days (with leap year handling via timedelta)
        total_days = gregorian_years * float(DAYS_PER_YEAR)
        days = int(total_days)
        fractional_days = total_days - days
        
        # 4. Convert fractional days to HH:MM:SS
        total_seconds = round(fractional_days * 86400)
        hours, remainder = divmod(total_seconds, 3600)
        minutes, seconds = divmod(remainder, 60)
        
        # 5. Apply to base date
        ...
        if timezone != "UTC":
            try:
                ...
                result = pytz.utc.localize(result).astimezone(tz)
            except Exception as e:
                return f"Timezone error: {str(e)}"
        
        # === Cosmic Context ===
        if verbose:
            elapsed = float(cu_diff)
            ctom_elapsed = max(0, elapsed - 2_800_000_000_000)
            ctom_remaining = 28_000_000_000 - ctom_elapsed
            
            context = (
                f"\n\n=== Cosmic Context ==="
                f"\n• Cosmic Breath Progress: {elapsed / TOTAL_BREATH:.2%}"
                f"\n• ZTOM ETA: {ctom_remaining / 1e9:.2f} billion years"
                f"\n• Current Phase: {'CTOM' if elapsed > 2.8e12 else 'BTOM'}"
                f"\n• Phase Progress: {ctom_elapsed / 28e9:.2%} of CTOM"
            )
        else:
            context = ""
            
        return f"{result.strftime('%Y-%m-%d %H:%M:%S %Z')}{context}"
        
    except OverflowError:
        return "ZTOM proximity alert (temporal overflow)"
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Standard conversion
    print("\n1. Basic conversion:")
    print(cu_to_gregorian(3_093_764_482_766.86, "America/Denver"))
    
    # With cosmic context
    print("\n2. With cosmic context:")
    print(cu_to_gregorian(3_093_764_482_766.86, "America/Denver", verbose=True))
    
    # Edge cases
    print("\n3. Edge cases:")
    print("Pre-BTOM:", cu_to_gregorian(1_000_000_000_000))
    print("ZTOM threshold:", cu_to_gregorian(3_108_000_000_000))
```
